...
#### ✅ Phase 2 검증 체크리스트
- [x] 좌측 사이드바로 대시보드와 청크 스튜디오가 매끄럽게 전환되는지 확인
- [x] 청크 스튜디오에서 청크를 선택했을 때 3열 에디터에 즉시 반영되는지 확인
- [x] 3열에서 수정한 내용이 2열 목록에 실시간 동기화되는지 확인
...
#### ✅ Phase 3 검증 체크리스트
- [x] 1개 청크 분할 시 2개의 독립된 청크로 정상 생성되고 부모 섹션에 등록되는지 확인
- [x] 2개 청크 병합 시 텍스트와 토큰 수가 합산되고 ID 정합성이 유지되는지 확인
- [x] 최종 JSONL 내보내기 시 분할/병합된 상태가 정확히 반영되는지 확인
