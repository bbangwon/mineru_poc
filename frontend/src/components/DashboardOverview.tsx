...
export const DashboardOverview: React.FC<DashboardOverviewProps> = ({
  ...
  // Global metrics with fallbacks
  const metrics = useMemo(() => {
    const totalPdfs = globalStats?.total_pdfs ?? pdfList.length;
    const parsedPdfs = globalStats?.parsed_pdfs ?? pdfList.filter((p) => p.etl_status === 'completed').length;
    const runningJobs = globalStats?.running_jobs ?? (isParsing ? 1 : 0);
    const totalChunks =
      globalStats?.total_chunks ??
      pdfList.reduce((acc, cur) => acc + (cur.stats?.total_chunks || 0), 0);
    const embeddedPdfs = globalStats?.embedded_pdfs ?? pdfList.filter((p) => p.is_embedded).length;

    return { totalPdfs, parsedPdfs, runningJobs, totalChunks, embeddedPdfs };
  }, [globalStats, pdfList, isParsing]);
  ...
  return (
    ...
    >
      ...
      {/* Drag overlay notice */}
      {isDragging && (
        <div className="fixed inset-0 z-50 bg-indigo-950/80 backdrop-blur-sm border-2 border-dashed border-indigo-400 flex flex-col items-center justify-center text-white pointer-events-none">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-sm">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          <button
            ...
            className="p-2 text-slate-400 hover:text-white bg-slate-800/80 border border-slate-700 rounded-xl transition-colors cursor-pointer"
            title="상태 새로고침"
          >
      ...
      {showSettings && (
        <div className="bg-slate-900/90 border border-indigo-500/30 rounded-2xl p-4 sm:p-5 shadow-lg backdrop-blur-md animate-in fade-in duration-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
            <div>
              <label className="block text-[11px] font-semibold text-slate-400 mb-1.5">MinerU 엔진</label>
              <select
                value={engine}
            ...
            <div>
              <label className="block text-[11px] font-semibold text-slate-400 mb-1.5">청킹 전략 (Strategy)</label>
              <select
                value={strategy}
                onChange={(e) => setStrategy(e.target.value)}
                className="w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-indigo-500"
              >
                <option value="general">일반 규정/기술 문서 (General Hierarchy)</option>
                <option value="legal">법률/규정 조문형 (Legal Article/Clause)</option>
              </select>
            </div>

            <div>
              <label className="block text-[11px] font-semibold text-slate-400 mb-1.5">파싱 범위</label>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => setAllPages(true)}
                  className={`flex-1 py-2 rounded-xl text-[11px] font-semibold border transition-all cursor-pointer ${
                    allPages
            ...
            {!allPages ? (
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 mb-1.5">구간 (시작 ~ 끝 페이지)</label>
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    min={0}
              ...
              <div className="flex items-end pb-2 text-slate-500 text-[11px]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Card 1: Total PDFs */}
        <div className="bg-slate-900/80 border border-slate-800/90 rounded-2xl p-4 flex items-center gap-3.5 shadow-sm">
          ...
          <div className="min-w-0">
            <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">전체 등록 문서</p>
            <div className="flex items-baseline gap-1.5 mt-0.5">
              <span className="text-xl sm:text-2xl font-bold text-white font-mono">{metrics.totalPdfs}</span>
        ...
        <div className="bg-slate-900/80 border border-slate-800/90 rounded-2xl p-4 flex items-center gap-3.5 shadow-sm">
          ...
          <div className="min-w-0">
            <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">ETL 파싱 완료</p>
            <div className="flex items-baseline gap-1.5 mt-0.5">
              <span className="text-xl sm:text-2xl font-bold text-emerald-400 font-mono">{metrics.parsedPdfs}</span>
        ...
        <div className="bg-slate-900/80 border border-slate-800/90 rounded-2xl p-4 flex items-center gap-3.5 shadow-sm">
          ...
          <div className="min-w-0">
            <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">총 생성 청크 수</p>
            <div className="flex items-baseline gap-1.5 mt-0.5">
              <span className="text-xl sm:text-2xl font-bold text-indigo-300 font-mono">
        ...
        <div
          ...
          className="bg-slate-900/80 border border-slate-800/90 hover:border-amber-500/40 rounded-2xl p-4 flex items-center gap-3.5 shadow-sm cursor-pointer transition-all group"
          title="Qdrant 연결 및 컬렉션 설정 열기"
        >
          <div className="w-11 h-11 rounded-xl bg-amber-500/15 border border-amber-500/30 group-hover:bg-amber-500/25 flex items-center justify-center text-amber-400 shrink-0 transition-colors">
            <Database className="w-5 h-5" />
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Qdrant 색인 완료</p>
            <div className="flex items-baseline gap-1.5 mt-0.5">
              <span className="text-xl sm:text-2xl font-bold text-amber-300 font-mono">{metrics.embeddedPdfs}</span>
      ...
      {(isParsing || (activeJob && (activeJob.status === 'running' || activeJob.status === 'pending'))) && (
        <div className="bg-gradient-to-r from-indigo-950 via-slate-900 to-indigo-950 border border-indigo-500/40 rounded-2xl p-4 sm:p-5 shadow-lg relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-start sm:items-center gap-3">
              <div className="w-9 h-9 rounded-xl bg-indigo-600/30 border border-indigo-400/50 flex items-center justify-center text-indigo-300 shrink-0 animate-pulse">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] px-1.5 py-0.2 bg-indigo-500/20 text-indigo-200 rounded font-mono border border-indigo-500/30">
                ...
                <p className="text-xs text-slate-300 mt-0.5 flex items-center gap-1.5">
                  ...
                  {activeJob?.elapsed_time ? (
                    <span className="text-slate-400 font-mono">({activeJob.elapsed_time}s 경과)</span>
                  ) : null}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 self-end sm:self-center">
              <span className="text-xs text-indigo-300 font-mono font-semibold animate-pulse">
                파싱 및 계층 청킹 진행 중...
              </span>
            </div>
          </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm">
        ...
        <div className="p-4 sm:p-5 border-b border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-3.5">
          {/* Status Filter Tabs */}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-950/60 text-slate-400 font-semibold border-b border-slate-800 select-none">
              ...
              {filteredList.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-12 text-center text-slate-500">
                    ...
                    <p className="text-xs text-slate-600 mt-1">상단의 '신규 PDF 등록' 버튼을 눌러 새 문서를 추가하세요.</p>
                  </td>
                </tr>
              ) : (
                  ...
                  return (
                    <tr
                      ...
                      className={`group transition-colors ${
                        isCurrent ? 'bg-indigo-950/20 hover:bg-indigo-950/30' : 'hover:bg-slate-800/30'
                      }`}
                    >
                      {/* 1. PDF Info */}
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="flex items-center gap-2 text-[11px] text-slate-500 mt-0.5">
                              ...
                              {item.last_modified && (
                                <>
                                  <span>•</span>
                                  <span className="text-slate-400">수정 {item.last_modified}</span>
                                </>
                              )}
                            </div>
                          </div>
                      ...
                      <td className="py-3.5 px-3">
                        {isRunningThis || item.etl_status === 'running' ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-sky-500/20 text-sky-300 border border-sky-500/30 animate-pulse">
                        ...
                        {item.stats ? (
                          <div className="space-y-1">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-slate-400 text-[11px]">청크</span>
                              {item.stats.tables_count > 0 && (
                                <span className="inline-flex items-center gap-0.5 text-[10px] px-1.5 py-0.2 rounded bg-amber-500/10 text-amber-300 border border-amber-500/20">
                      ...
                      <td className="py-3.5 px-3">
                        {item.has_saved_edit ? (
                          <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md bg-purple-500/15 text-purple-300 border border-purple-500/30 font-semibold">
                            <FileCode2 className="w-3 h-3 text-purple-400" />
                            수정본 저장됨
                          </span>
                        ) : item.etl_status === 'completed' ? (
                          <span className="text-[10px] text-slate-400 font-medium">기본 파싱 원형</span>
                        ) : (
                          <span className="text-slate-600">-</span>
                        )}
                      </td>
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 text-white rounded-xl text-xs font-semibold shadow-sm flex items-center gap-1 transition-all cursor-pointer"
                          ...
                          <button
                            ...
                            disabled={isParsing}
                            className={`p-1.5 rounded-xl border transition-all cursor-pointer disabled:opacity-40 ${
                              item.etl_status === 'completed'
