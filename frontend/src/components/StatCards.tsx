...
import { FolderTree, Boxes, Table2, AlignLeft, Type } from 'lucide-react';
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-5 gap-3.5">
      ...
      <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-xs hover:border-slate-300 transition">
        ...
        <p className="text-[10px] text-slate-400 mt-0.5">Heading 계층 분할</p>
      </div>
      ...
      <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-xs hover:border-slate-300 transition">
        <div className="flex items-center justify-between text-slate-400 mb-1">
          <span className="text-[11px] font-semibold uppercase tracking-wider">문단 청크 (Text)</span>
          <AlignLeft className="w-4 h-4 text-slate-400" />
        </div>
        <div className="text-2xl font-bold text-slate-800 tracking-tight">
          {stats ? stats.paragraph_chunks.toLocaleString() : '-'}
        </div>
        <p className="text-[10px] text-slate-400 mt-0.5">자연 문맥 문단</p>
      </div>
